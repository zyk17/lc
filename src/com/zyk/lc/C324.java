...
public class C324 {
    ...
    public static void wiggleSort(int[] nums) {
        int N = nums.length;
        // 通过快排 partition 可划分为 <= index   ==index       >= index
    ...
    private static int findIndexNum(int[] arr, int L, int R, int index) {
        ...
        int[] range = null;
        while (L < R) {
            pivot = arr[L + (int) (Math.random() * (R - L + 1))];       // 随机选一个数
            range = partition(arr, L, R, pivot);        // 拿到随机数的，左右区间。且左区间左都小，右区间右都大
